...
// Hero copy intentionally describes both the physical flower shop and delivery service.
// "Цветы, которые говорят за вас" is the current approved storefront message.
...
export default function PublicBoutique2() {
  ...
  return <div className="bt2-site" id="top">
    ...
    <main>
      ...
      <section className="bt2-section bt2-catalog" id="catalog"><SectionTitle>Каталог</SectionTitle><p className="bt2-sub">Готовые букеты, которые можно заказать сегодня</p>{loading && <div className="bt2-loading">Обновляем каталог…</div>}<div className="bt2-grid">{products.slice(0, 6).map((p, i) => <button key={p.id} className="bt2-card" onClick={() => setSelected(p)}><span className="bt2-card-photo"><img src={img(p, i)} alt={p.alt || p.name} /></span><span className="bt2-card-meta"><span><strong>{p.name}</strong><small>{categories.find(c => c.id === p.category_id)?.name || "Букет"}</small></span><b>{money(p.price)}</b><em>Заказать</em></span></button>)}</div><Button onClick={() => document.getElementById("bestsellers")?.scrollIntoView({ behavior: "smooth" })}>Открыть ещё</Button></section>
      <section className="bt2-section bt2-why" id="about"><SectionTitle>Почему именно мы?</SectionTitle><p className="bt2-sub">Наши букеты дарят вам самые яркие эмоции</p><div className="bt2-why-grid"><div className="bt2-large-photo"><img src={img(products[3] || fallbackProducts[3], 3)} alt="Авторский букет" /></div><div className="bt2-reasons"><article><HeartHandshake /><h3>Доверие</h3><p>Внимательно собираем каждый заказ и остаёмся на связи.</p></article><article><Truck /><h3>Доставка</h3><p>Передаём букеты через {settings.delivery_method} в удобное время.</p></article><article><ShieldCheck /><h3>Качество</h3><p>Работаем со свежими цветами и проверяем каждую композицию.</p></article><article><Gift /><h3>Подарок</h3><p>Поможем подобрать открытку и красивое оформление.</p></article></div></div></section>
